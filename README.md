...
| `OPENCLAW_DISABLE_DEVICE_AUTH` | `false` | Set to `true` to disable Control UI device pairing checks and rely on gateway token auth only. Intended for cloud deployments where interactive pairing is not feasible. |
