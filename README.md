...
| `OPENCLAW_DISABLE_DEVICE_AUTH` | `false` | Set to `true` to disable device pairing checks for all gateway clients and rely on gateway token auth only. Intended for cloud deployments where interactive pairing is not feasible. |
